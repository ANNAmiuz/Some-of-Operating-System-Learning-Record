...
#define OUTFILE0 "./snapshot0.bin"
#define OUTFILE1 "./snapshot1.bin"
#define OUTFILE2 "./snapshot2.bin"
#define OUTFILE3 "./snapshot3.bin"
...
__device__ __managed__ uchar results0[STORAGE_SIZE];
__device__ __managed__ uchar results1[STORAGE_SIZE];
__device__ __managed__ uchar results2[STORAGE_SIZE];
__device__ __managed__ uchar results3[STORAGE_SIZE];
...
__global__ void mykernel(int input_size) {
  ...
  if (threadIdx.x == 0) {
    user_program(&vm, input, results0, input_size);
    __syncthreads();
    __syncthreads();
    __syncthreads();
    __syncthreads();
  ...
  if (threadIdx.x == 1) {
    ...
    vm_init(&vm, data, storage, pt, &pagefault_num, PAGE_SIZE,
      INVERT_PAGE_TABLE_SIZE, PHYSICAL_MEM_SIZE, STORAGE_SIZE,
      PHYSICAL_MEM_SIZE / PAGE_SIZE);
    user_program(&vm, input, results1, input_size);
    __syncthreads();
    __syncthreads();
    __syncthreads();
  }
  if (threadIdx.x == 2) {
    ...
    vm_init(&vm, data, storage, pt, &pagefault_num, PAGE_SIZE,
      INVERT_PAGE_TABLE_SIZE, PHYSICAL_MEM_SIZE, STORAGE_SIZE,
      PHYSICAL_MEM_SIZE / PAGE_SIZE);
    user_program(&vm, input, results2, input_size);
    __syncthreads();
  }
  if (threadIdx.x == 3) {
    __syncthreads();
    ...
    vm_init(&vm, data, storage, pt, &pagefault_num, PAGE_SIZE,
      INVERT_PAGE_TABLE_SIZE, PHYSICAL_MEM_SIZE, STORAGE_SIZE,
      PHYSICAL_MEM_SIZE / PAGE_SIZE);
    user_program(&vm, input, results3, input_size);
  }
}
...
int main() {
  ...
  /* Launch kernel function in GPU, with FOUR threads
  ...
  write_binaryFile(OUTFILE0, results0, input_size);
  write_binaryFile(OUTFILE1, results1, input_size);
  write_binaryFile(OUTFILE2, results2, input_size);
  write_binaryFile(OUTFILE3, results3, input_size);
