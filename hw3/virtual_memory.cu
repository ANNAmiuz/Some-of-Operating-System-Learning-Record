...
__device__ uchar vm_read(VirtualMemory *vm, u32 addr) {
  ...
  for (int i = 0; i < vm->PAGE_ENTRIES; ++i)
  {
    ...
    if (page_number_in_PT == virtual_page_number)
    {
      page_hit = true;
      physical_page_number = i;
      break;
  ...
  if (page_hit == false)
  {
    ...
    if (is_full == false)
    {
      ...
      vm->invert_page_table[physical_page_number] = (0 << 31) | (0 << 12) | virtual_page_number;
...
__device__ void vm_write(VirtualMemory *vm, u32 addr, uchar value) {
  ...
  if (page_hit == false)
  {
    ...
    {
      page_swap(vm, physical_page_number<<5, virtual_page_number<<5, vm->PAGESIZE, StorageToBuffer);       // insert in
      vm->invert_page_table[physical_page_number] = (0 << 31) | (1 << 30)| (0 << 12) | virtual_page_number;
