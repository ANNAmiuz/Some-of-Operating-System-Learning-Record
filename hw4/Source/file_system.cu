...
__device__ void set_FCB_modify_time(uchar *FCB_start, u32 time) {
  #ifdef DEBUG
  printf("[DEBUG] set modify time as %d\n", time);
  #endif
  uchar c1 = (time & 0xff000000) >> 24;
  uchar c2 = (time & 0xff0000) >> 16;
  uchar c3 = (time & 0xff00) >> 8;
  ...
  #ifdef DEBUG
  printf("[DEBUG] after set modify time, time as %d\n", get_FCB_modify_time(FCB_start));
  #endif
}
...
__device__ void compact(FileSystem * fs) {
  ...
  count = end - start + 1;
  #ifdef DEBUG
  printf("compact start: %d; end: %d, size: %d, used size: %d, first %d, last: %d \n", start, end, count, used, first, last);
  #endif
  ...
  uchar * src_start, * src_end, * dest_start, * dest_end;
  ...
  //     move_buf[i * fs->STORAGE_BLOCK_SIZE + j] = *()
  //   }
  // }
  // printf("before copying: the content is: %s\n", move_buf);
  // #endif
...
__device__ u32 fs_open(FileSystem *fs, char *s, int op)
{
  ...
  if (found == true) {
    ...
    if (op == G_READ)
      ...
      #endif
      //printf("[DEBUG] file %s create time: %d\n", s, gtime);
...
__device__ void fs_gsys(FileSystem *fs, int op)
{
  ...
  for (int i = 0; i < fs->FCB_ENTRIES; ++i) {
    ...
    if (isValid) {
      ...
      printf("file %s after copy into sorting array: idx: %d %d; size: %d %d; create time: %d %d, modify time: %d %d\n",
      name_buf, i, array[count]->FCB_idx, get_FCB_size(current_FCB), array[count]->file_size, get_FCB_create_time(current_FCB),
  ...
  // by size
    ...
    for (int i = 0; i < count; ++i) {
      ...
      //printf("[DEBUG] file %s with idx %d, size %d, create time %d\n", name_buf, array[i]->FCB_idx, file_size, array[i]->create_time);
